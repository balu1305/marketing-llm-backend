...
const seedPersonas = async () => {
  try {
    ...
    const existingPersonas = await Persona.countDocuments({ isPredefined: true });
    
    if (existingPersonas > 0) {
      console.log(`⚠️  Found ${existingPersonas} existing predefined personas. Skipping...`);
      return;
    }
    ...
    console.log(`✅ Successfully seeded ${personas.length} predefined personas`);
    
    // Display seeded personas
    personas.forEach((persona, index) => {
      console.log(`   ${index + 1}. ${persona.name} (${persona.demographics.age})`);
    });
