...
const testCampaign = {
  ...
  description: 'Launch campaign for our new innovative product targeting tech enthusiasts',
  objective: 'awareness',
  budget: 5000,
  currency: 'USD',
  ...
  generationSettings: {
    ...
    creativityLevel: 7,
    includeVisuals: true,
    customInstructions: 'Focus on the innovative aspects and productivity benefits'
  }
};
...
const getPersonas = async () => {
  ...
  if (result.success && result.data.data.personas.length > 0) {
    ...
    console.log(`✅ Found ${result.data.count} personas. Using: ${result.data.data.personas[0].name}`);
    return true;
  } else {
...
const createTestCampaign = async () => {
  ...
  const result = await apiRequest('POST', '/campaigns', campaignData, authToken);
  ...
  if (result.success) {
    ...
    console.log('❌ Failed to create campaign:', result.error.message || result.error);
    return false;
  }
};
...
const testCampaignOperations = async () => {
  ...
  // Get campaigns
  const getCampaignsResult = await apiRequest('GET', '/campaigns', null, authToken);
  if (getCampaignsResult.success) {
    console.log(`✅ Get campaigns: Found ${getCampaignsResult.data.data.campaigns.length} campaigns`);
  } else {
    console.log('❌ Get campaigns failed');
  }
  
  // Get single campaign
  const getCampaignResult = await apiRequest('GET', `/campaigns/${testCampaignId}`, null, authToken);
  ...
  // Get campaign stats
  const getStatsResult = await apiRequest('GET', '/campaigns/stats', null, authToken);
  ...
  const getDashboardResult = await apiRequest('GET', '/campaigns/dashboard', null, authToken);
...
const testAIContentGeneration = async () => {
  ...
  const aiStatusResult = await apiRequest('GET', '/content/ai-status', null, authToken);
  if (aiStatusResult.success) {
    console.log(`✅ AI Status: ${aiStatusResult.data.data.message}`);
    
    if (!aiStatusResult.data.data.available) {
      console.log('⚠️  AI service not available, skipping content generation tests');
      return;
    }
  } else {
  ...
  const emailResult = await apiRequest('POST', '/content/generate-email', {
  ...
  if (emailResult.success) {
    ...
    console.log('⚠️  Email generation expected to fail without OpenAI API key:', emailResult.error.message);
  }
  
  // Test social content generation
  const socialResult = await apiRequest('POST', '/content/generate-social', {
    campaignId: testCampaignId,
    personaId: demoPersonaId,
    platform: 'linkedin'
  }, authToken);
  ...
  if (socialResult.success) {
    ...
    console.log('⚠️  Social generation expected to fail without OpenAI API key:', socialResult.error.message);
  }
};
...
The Innovation Team`,
  ...
  const result = await apiRequest('POST', `/campaigns/${testCampaignId}/content`, manualContent, authToken);
...
const updateCampaign = async () => {
  ...
  const result = await apiRequest('PUT', `/campaigns/${testCampaignId}`, updateData, authToken);
...
const runPhase2Tests = async () => {
  console.log('🚀 Starting Phase 2 Tests - Campaign Management & Content Generation');
  ...
  try {
    ...
    const finalCheck = await apiRequest('GET', `/campaigns/${testCampaignId}`, null, authToken);
    if (finalCheck.success) {
      console.log(`✅ Final check: Campaign has ${finalCheck.data.data.campaign.content.length} content items`);
      passedTests++;
    }
  ...
  console.log('=' .repeat(70));
  console.log(`📊 Phase 2 Test Results: ${passedTests}/${totalTests} tests passed`);
