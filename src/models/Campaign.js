...
const contentSchema = new mongoose.Schema({
  ...
  platform: {
    type: String,
    enum: ['email', 'linkedin', 'facebook', 'twitter', 'instagram', 'youtube', 'tiktok'],
...
const campaignSchema = new mongoose.Schema({
  ...
  objective: {
    type: String,
    enum: ['awareness', 'engagement', 'conversion', 'retention', 'lead_generation'],
    required: [true, 'Campaign objective is required']
  },
  status: {
    type: String,
    enum: ['draft', 'generating', 'active', 'paused', 'completed', 'archived'],
    default: 'draft',
    index: true
  },
  budget: {
  ...
  tone: {
    type: String,
    enum: ['professional', 'casual', 'friendly', 'urgent', 'humorous', 'inspiring', 'authoritative'],
...
campaignSchema.pre('save', function(next) {
  ...
  if (this.metrics.totalImpressions > 0) {
    this.metrics.ctr = (this.metrics.totalClicks / this.metrics.totalImpressions) * 100;
  }
