...
Format: Headline (under 30 characters), Description (under 90 characters), Call-to-action (under 20 characters)`
  ...
  async generateEmailContent(persona, campaign) {
    try {
      ...
      const subjectPrompt = this.prompts.email.subject.replace(/\{(\w+)\}/g, (match, key) => {
      ...
      const subjectResponse = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are an expert email marketing copywriter. Generate compelling, high-converting email subject lines."
          },
          {
            role: "user",
      ...
      // Generate email body
      const bodyPrompt = this.prompts.email.body.replace(/\{(\w+)\}/g, (match, key) => {
      ...
      const bodyResponse = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are an expert email marketing copywriter. Write engaging, personalized email content that converts."
          },
          {
            role: "user",
            content: bodyPrompt
      ...
      const qualityScore = await this.scoreContentQuality(contentBody, persona, 'email');
  ...
  async generateSocialContent(persona, campaign, platform) {
    try {
      ...
      const prompt = this.prompts.social.post.replace(/\{(\w+)\}/g, (match, key) => {
        return context[key] || match;
      });
      ...
      // Score the content quality
      const qualityScore = await this.scoreContentQuality(contentBody, persona, 'social_post');
  ...
  async generateAdCopy(persona, campaign, platform = 'google-ads') {
    try {
      ...
      const prompt = this.prompts.adCopy.short.replace(/\{(\w+)\}/g, (match, key) => {
        return context[key] || match;
      });
      ...
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are an expert digital advertising copywriter. Create high-converting ad copy that maximizes click-through rates and conversions."
          },
          {
            role: "user",
            content: prompt
      ...
      // Score the content quality
      const qualityScore = await this.scoreContentQuality(contentBody, persona, 'ad_copy');
...
Return only a number between 0-100.`;
      ...
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are a marketing content quality analyst. Provide objective scores based on marketing best practices."
          },
          {
            role: "user",
   ...
   */
  async generateContentVariations(persona, campaign, contentType, variations = 2) {
    const promises = [];
    ...
    for (let i = 0; i < variations; i++) {
      if (contentType === 'email') {
        ...
        const platforms = campaign.generationSettings?.platforms || ['linkedin'];
        promises.push(this.generateSocialContent(persona, campaign, platforms[0]));
  ...
  buildContext(persona, campaign, platform = '') {
    return {
      ...
      demographics: `Age: ${persona.demographics?.age || 'N/A'}, Income: ${persona.demographics?.income || 'N/A'}, Location: ${persona.demographics?.location || 'N/A'}`,
